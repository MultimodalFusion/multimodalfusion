...
'''
def initialize_df(slides, seg_params, filter_params, vis_params, patch_params, use_heatmap_args=False):
	...
	if isinstance(slides, pd.DataFrame):
		...
		slide_ids = slides
	default_df_dict = {'slide_id': slide_ids, 'process_patches': np.full((total), 1, dtype=np.int)}
	...
	default_df_dict.update({
		'status_patches': np.full((total), 'tbp'),
		...
		'level0_mag': np.full((total), -1, dtype=np.uint32),
		'mag_level': np.full((total), -1, dtype=np.uint32),
		'patch_level':  np.full((total), int(patch_params['patch_level']), dtype=np.uint32),
		...
		'custom_downsample': np.full((total), int(patch_params['custom_downsample']), dtype=np.uint32),
		'save_patch':  np.full((total), bool(patch_params['save_patch']), dtype=bool),

		### features params
		'process_features': np.full((total), 1, dtype=np.int),
		'status_features': np.full((total), 'tbp'),

		### hovernet params
		'process_hovernet': np.full((total), 1, dtype=np.int),
		'status_hovernet': np.full((total), 'tbp'),
		})
