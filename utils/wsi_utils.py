...
import os
import pdb
from utils.util_classes import Mosaic_Canvas
import PIL
...
def filterPatch(patch, patch_filter_params={'isWhitePatch': {'satThresh': 5},
                                     'isBlackPatch': {'rgbThresh': 40},
                                     'isUniformPatch': {'thresh': 100}
                                     }):
    for k in patch_filter_params.keys():
        if eval(k)(patch=patch, **patch_filter_params[k]):
            return 1
    return 0
...
def isUniformPatch(patch, thresh=100):
    if isinstance(patch, PIL.Image.Image):
        patch = np.array(patch)
    patch = patch / 255
    blur1 = cv2.GaussianBlur(patch, (7, 7), 1)
    blur2 = cv2.GaussianBlur(patch, (31, 31), 2)
    ssd_blur = np.sum((blur1 - blur2)**2)
    if ssd_blur < 100:
        return True
    else:
        return False
...
def _save_hdf5(output_path, asset_dict, mode='a'):
    file = h5py.File(output_path, mode)
...
def initialize_hdf5_bag(first_patch, save_patch=True, save_coord=False):
    ...
    if save_coord:
        ...
        coord_dset[:] = (x,y)
        coord_dset.attrs['patch_size'] = img_patch.shape[0]
        coord_dset.attrs['patch_level'] = patch_level
        coord_dset.attrs['wsi_name'] = name
        coord_dset.attrs['downsample'] = downsample
        coord_dset.attrs['level_dim'] = level_dim
        coord_dset.attrs['downsampled_level_dim'] = downsampled_level_dim
