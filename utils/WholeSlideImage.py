...
import h5py
import math
from utils.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, screen_coords, isBlackPatch, isWhitePatch, to_percentiles
from utils.wsi_utils import isUniformPatch

import itertools
from utils.util_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, Contour_Checking_fn
...
class WholeSlideImage(object):
	...
	def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
		...
		if vis_level < len(self.level_downsamples):
	...
	def createPatches_bag_hdf5(self, save_path, patch_level=0, patch_size=256, step_size=256, save_patch=True, save_coord=True, pbar=None, **kwargs):
		...
		for idx, cont in enumerate(contours):
			patch_gen = self._getPatchGenerator(cont, idx, patch_level, save_path, patch_size, step_size, save_patch=save_patch, **kwargs)
			...
			if self.hdf5_file is None:
				...
				file_path = initialize_hdf5_bag(first_patch, save_patch=save_patch, save_coord=save_coord)
	...
	def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
		white_black=True, white_thresh=15, black_thresh=50, contour_fn='four_pt', use_padding=True, save_patch=True):
		...
		for y in range(start_y, stop_y, step_size_y):
			for x in range(start_x, stop_x, step_size_x):
				...
				if white_black:
					if isBlackPatch(np.array(patch_PIL), rgbThresh=black_thresh) or isWhitePatch(np.array(patch_PIL), satThresh=white_thresh) or isUniformPatch(np.array(patch_PIL)): 
						continue
				...
				if not save_patch:
					...
					'patch_PIL':None, 'name':self.name, 'save_path':save_path}
				else:
					if custom_downsample > 1:
						target_patch_size = patch_PIL.size[0] * custom_downsample 
	...
	def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, pbar=None, **kwargs):
		save_path_hdf5 = os.path.join(save_path, str(self.name) + '_patches.h5')
		#print("Creating patches for: ", self.name, "...",)
		elapsed = time.time()
		...
		for idx, cont in enumerate(self.contours_tissue):
			...
			asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], patch_level, save_path, patch_size, step_size, pbar=None, **kwargs)
			if len(asset_dict) > 0:
				if init:
	...
	def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
		contour_fn='four_pt', use_padding=True, top_left=None, bot_right=None, pbar=None, **kwargs):
		start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
  ...
  #           	break
	...
	def visHeatmap(self, scores, coords, vis_level=0, 
		...
		if isinstance(cmap, str):
			...
			#top = mpl.cm.get_cmap('RdYlBu_r', 128)
			#bottom = mpl.cm.get_cmap('RdYlBu_r', 128)
			#newcolors = np.vstack((bottom(np.linspace(0, 0.3, 128)),
			                       #top(np.linspace(0.6, 1, 128))))
			#cmap = ListedColormap(newcolors, name='OrangeBlue')
			cmap = plt.get_cmap('RdYlBu_r')#.reversed()
		#import pdb;pdb.set_trace()
	...
	def fetch_mag_patching_params(self, mag_level=40, patch_size=256, step_size=256, predict_only=True, dec_prec=1):
		try:
			mpp = float(self.wsi.properties['openslide.mpp-x'])
		except:
			mpp = -1
		try:
			op = self.wsi.properties['openslide.objective-power']
		except:
			op = -1
		try:
			mag = self.wsi.properties['aperio.AppMag']
		except:
			mag = -1    
		level0_mag = -1
		if mpp < 0.3:
		...
		new_patch_level, new_patch_size, new_step_size = None, None, None
		all_downsamples = [xy[0] for xy in self.level_downsamples]
		# round to specified decimal precision
		if dec_prec >= 0:
			all_downsamples = list(np.around(all_downsamples, dec_prec))

		custom_downsample = int(level0_mag / mag_level)
		if custom_downsample in all_downsamples:
			new_patch_level = all_downsamples.index(custom_downsample)
			new_patch_size = patch_size
			new_step_size = step_size
			custom_downsample = None
		else:
			new_patch_level = 0
			new_patch_size = int(patch_size * custom_downsample)
			new_step_size = int(step_size * custom_downsample)
		
		if predict_only:
			return level0_mag, new_patch_level, new_patch_size, new_step_size, custom_downsample
		else:
			return mpp, int(op), int(float(mag)), level0_mag, new_patch_level, new_patch_size, new_step_size, custom_downsample


	def print_WSI_info(self, desired_mag_level=20):
	    level0_mag, new_patch_level, new_patch_size, new_step_size, mag_downsample = self.fetch_mag_patching_params(mag_level=20)
	    level_downsamples = [int(l[0]) for l in self.level_downsamples]
	    print("Level 0 in this WSI corresponds to: %dX magnification" % level0_mag)
	    print("Downsamples Available:", ', '.join(['1/%d'%l for l in level_downsamples]))
	    print("Downsamples as Mags:", ', '.join(['%0.2fX'%(level0_mag/l) for l in level_downsamples]))
	    print("Image Dimensions across Levels:", self.level_dim)
	    if mag_downsample == None:
	        print('20X Mag is in the Downsamples - Corresponds to Level %d' % new_patch_level)
	    else:
	        print('20X not available. Patches need to be read at Level %d with %d times the desired patch size.' % (new_patch_level, mag_downsample))
