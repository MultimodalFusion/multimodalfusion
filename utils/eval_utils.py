...
from models.model_genomic import MaxNet,MaxNet_captum
...
def initiate_model(args, ckpt_path=None):
    ...
    if args.model_type =='path_attention_mil':
        ...
        model_dict.update({'modalities':args.modalities})#, 'radio_mil_type':args.radio_mil_type})
        model = MIL_Attention_fc_surv_radio(**model_dict)

    elif args.model_type =='max_net':
        model_dict = {'input_dim': args.omic_input_dim, 'model_size_omic': args.model_size_omic,
         'n_classes': args.n_classes,'bag_loss': args.bag_loss}
        model = MaxNet_captum(**model_dict)
    else:
        raise NotImplementedError
    """
    elif args.model_type =='mm_attention_mil':
        model_dict.update({ 'input_dim': args.omic_input_dim, 'mode': args.mode,'fusion': args.fusion, 
            'model_size_wsi':args.model_size_wsi, 'model_size_mri':args.model_size_mri,'model_size_omic':args.model_size_omic,
            'gate_radio': args.gate_radio,'gate_path': args.gate_path, 'gate_omic': args.gate_omic, 'n_classes': args.n_classes})
...
def initiate_pretrained_model(args, ckpt_path=None):
    ...
    weights = torch.load(os.path.join(ckpt_path))
...
def eval2(datasets: tuple, cur: int, args):
    """   
